...
class JobAssigneeHook
{
    ...
    public static async indexQueryHook(query: object, request: object, slug:string = '')
    {
        ...
        query.preload('User').preload('Job').orderBy('id','desc')
    }
    ...
    {
        let slug      = await JobAssignee.generateSlug(params.crew_name);
        params.slug       = slug
...
module.exports = JobAssigneeHook;
