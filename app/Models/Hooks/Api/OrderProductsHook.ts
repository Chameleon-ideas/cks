...
class OrderProductsHook
{
    ...
    public static async indexQueryHook(query: object, request: object, slug:string = '')
    {
        ...
        query.preload('CreatedBy').preload('Product').where('order_id',0).where('created_by_id',request.user().id).orderBy('id','desc')
    }
    ...
    public static async beforeCreateHook(request: object, params:object)
    {
        ...
        let product = await Product.query().where('id',params.product_id).first();
        let slug      = await OrderProducts.generateSlug(request.user().name+product.title);
...
module.exports = OrderProductsHook;
