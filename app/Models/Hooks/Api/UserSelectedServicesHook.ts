...
class UserSelectedServicesHook
{
     /**
    ...
    public static async indexQueryHook(query: object, request: object, slug:string = '')
    {
        ...
        query.preload('User').preload('Services').orderBy('id','desc')
    }
    ...
    {
        let slug      = await UserSelectedServices.generateSlug('slctd_'+params.user_id+params.service_id);
        params.slug       = slug
...
module.exports = UserSelectedServicesHook;
