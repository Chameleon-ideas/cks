...
class OrderHook
{
    ...
    public static async indexQueryHook(query: object, request: object, slug:string = '')
    {
        ...
        query.preload('CreatedBy').preload('OrderProducts',function(qry){
            qry.preload('CreatedBy').preload('Product')
        }).orderBy('id','desc')
    }
    ...
    public static async beforeCreateHook(request: object, params:object)
    {
        ...
        let slug      = await Order.generateSlug(request.user().name+request.user().id);
        var now = new Date();
        var dateString = moment(now).format('YYYY-MM-DD');
        params.created_by_id = request.user().id
        params.order_date = dateString
        params.slug       = slug
    ...
    {
        await OrderProducts.updateOrderProduct({'order_id':record.id},{'created_by_id':request.user().id,'order_id':0})
