...
export default class ProductController extends RestController
{
    ...
    protected async beforeDestoryLoadModel()
    {
      ...
      let get_product = await Product.query().where('slug',slug).first();
      let check_product = await OrderProducts.query().where('product_id',get_product.id).getCount();
      if(check_product > 0){
        this.__is_error = true;
          return this.sendError(
              'Validation Message',
              { message: 'Product already associated with orders' },
              400
          );
      }
