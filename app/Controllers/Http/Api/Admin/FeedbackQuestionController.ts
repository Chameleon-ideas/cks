...
export default class FeedbackQuestionController extends RestController
{
    ...
    constructor() {
        ...
        this.__resource = "FeedbackQuestion";
    ...
    private async storeValidation()
    {
        ...
        try{
          validator = await this.__request.validate({ schema: validationRules,messages: {
                      required: '{{ field }} is required to feedback'
          }
         })
        } catch(error){
    ...
    protected async beforeDestoryLoadModel()
    {
          ...
          return this.sendError(
              'Validation Message',
              { message: "You Can't delete this question because already associated with another user feedback" },
              400
          );
    }
